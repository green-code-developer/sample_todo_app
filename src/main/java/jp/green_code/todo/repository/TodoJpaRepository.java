...
public interface TodoJpaRepository extends Repository<TodoEntity, Long> {
  ...
  String FORM_SEARCH = """
      from todo
      where
        (:word is null or :word = '' or detail like concat('%',:word,'%'))
        and (:status is null or :status = '' or todo_status = :status)
        and (:deadlineFrom is null or :deadlineFrom = '' or cast(:deadlineFrom as timestamp) <= deadline)
        and (:deadlineTo is null or :deadlineTo = '' or deadline <= cast(:deadlineTo as timestamp))
      """;
